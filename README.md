...
* [`src/clojure_dice_frontend/`](src/clojure_dice_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clojure_dice_frontend/core.cljs): contains the SPA entry point, `init`
* [`test/clojure_dice_frontend/`](test/clojure_dice_frontend/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `clojure-dice-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clojure_dice_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure-dice-frontend.example
  (:require [clojure-dice-frontend.config :as config])
